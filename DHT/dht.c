...
void get_temp_hum(void) {
	if(dht_gettemperaturehumidity(&temperature, &humidity) != -1)  {
								...
								#elif DHT_FLOAT == 1
								dtostrf(temperature, 2, 1, printbuff);
								#endif
								lcd_locate(0,0); lcd_str("temp: "); lcd_str(printbuff); lcd_str(" oC");
								#if DHT_FLOAT == 0
								itoa(humidity, printbuff, 10);
								#elif DHT_FLOAT == 1
								dtostrf(humidity, 2, 1, printbuff);
								#endif
								lcd_locate(1,0); lcd_str("hum: "); lcd_str(printbuff); lcd_str(" %RH");

							} else {
								lcd_locate(0,0); lcd_str("error");
							}
}
